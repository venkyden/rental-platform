...
@mcp.tool()
async def switch_user_segment(
    email: str, new_segment: str, confirm: bool = False
) -> str:
    """
    ...
    if new_segment not in valid_segments:
        return (
            f"Error: Invalid segment '{new_segment}'. Must be one of {valid_segments}."
        )
...
@mcp.tool()
async def generate_mock_lease(
    property_id: str, tenant_email: str, lease_type: str = "meuble"
) -> str:
    """
    ...
    async with AsyncSessionLocal() as db:
        ...
        if prop.size_sqm:
            if float(prop.size_sqm) < 9.0:
                issues.append(
                    f"❌ **SURFACE AREA ILLEGAL**: {prop.size_sqm}m² is below the legal minimum of 9m² for a primary residence."
                )
            else:
                passes.append(
                    f"✅ Surface Area: {prop.size_sqm}m² (Compliant with 9m² minimum)"
                )
        else:
            warnings.append(
                "⚠️ Surface area (size_sqm) is not specified. Cannot verify minimum decency criteria."
            )
        ...
        if prop.dpe_rating:
            ...
            if rating in ["G"]:
                issues.append(
                    f"❌ **PASSOIRE THERMIQUE (G)**: Banned from renting starting Jan 1, 2025. Rent is currently strictly frozen."
                )
            elif rating == "F":
                warnings.append(
                    f"⚠️ **PASSOIRE THERMIQUE (F)**: Rent is frozen. Will be banned from renting starting Jan 1, 2028."
                )
            ...
            warnings.append(
                "⚠️ Missing DPE rating. DPE calculation is mandatory for all rental listings."
            )
        ...
        if prop.city and prop.city.lower() in [
            "paris",
            "lille",
            "lyon",
            "villeurbanne",
            ...
            if prop.size_sqm and prop.monthly_rent:
                ...
                if (
                    price_per_sqm > 35.0
                ):  # 35 EUR/m2 is a completely arbitrary high threshold for generic heuristic
                    warnings.append(
                        f"⚠️ **RENT CONTROL RISK**: {prop.city} is subject to rent control (Encadrement des loyers). At €{price_per_sqm:.2f}/m² HC, this rent is statistically very high and may require a solid legal 'Complément de Loyer' justification."
                    )
                else:
                    passes.append(
                        f"✅ Rent Price/m²: €{price_per_sqm:.2f} (Within statistical norm for {prop.city})"
                    )
        ...
        if prop.guarantor_required and prop.accepted_guarantor_types:
            if "visale" not in [gt.lower() for gt in prop.accepted_guarantor_types]:
                warnings.append(
                    "⚠️ **GUARANTOR POLICY**: Landlord requires a guarantor but does not explicitly list 'Visale'. Refusing Visale can be viewed negatively and limits the tenant pool."
                )
        ...
        if not issues and not warnings:
            report.append(
                "> 🎉 **100% COMPLIANT**: No obvious legal issues detected from available data."
            )
