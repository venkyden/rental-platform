...
def get_auth_token():
    ...
    try:
        ...
        if r.status_code == 201:
            ...
            r = requests.post(f"{BASE}/auth/login", data={"username": TEST_EMAIL, "password": TEST_PASS})
            if r.status_code == 200:
...
def test_verification_flow(token):
    ...
    r = requests.post(f"{BASE}/properties/{prop_id}/media-session", headers=headers, params={"gps_radius": 500, "expires_in_hours": 24})
    if r.status_code != 200:
...
def stress_test_upload(verification_code, num_requests=20):
    ...
    def single_upload(i):
        ...
        try:
            ...
            files_fresh = {'file': ('test.jpg', open('test_upload.jpg', 'rb'), 'image/jpeg')}
            r = requests.post(url, params={"verification_code": verification_code, "metadata": metadata}, files=files_fresh)
...
def test_security_access(prop_id, token):
    ...
    r = requests.post(f"{BASE}/properties/media/upload", params={"verification_code": "INVALID_CODE", "metadata": valid_meta}, files=files)
    if r.status_code in [400, 403, 404]: # Assuming 400 'Session invalid' or 404 'Session not found'
