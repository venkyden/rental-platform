...
def test_gli_rate_limit():
    """GLI quote endpoint must reject the (limit+1)th request with 429."""
    user = make_mock_user("landlord")
    ...
    with patch("app.routers.verification.cache") as mock_cache, \
         patch("app.routers.verification.feature_flag_service") as mock_ff:
        ...
        for i in range(10):
            ...
            assert res.status_code == 200, (
                f"Request {i + 1} should succeed, got {res.status_code}: {res.text}"
            )

        # 11th request must be rejected
        res = client.post("/verification/gli/quote", json=payload)
        assert res.status_code == 429, (
            f"11th request must return 429, got {res.status_code}"
        )
