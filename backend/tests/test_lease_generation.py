...
def test_safety_invariant_standardized_text_unchanged():
    """Fillable template minus {{tokens}} == verbatim model minus [...] blanks."""
    verbatim = _BLANK_RE.sub("", registry.load_model("vide"))
    fillable = _TOKEN_RE.sub("", _BLANK_RE.sub("", registry.load_fill_model("vide")))
    assert fillable == verbatim, "standardized clause text diverged from the official model"
