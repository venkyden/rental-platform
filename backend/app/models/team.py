...
class TeamMember(Base):
    ...
    member_user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"), nullable=True, index=True)
    ...
    invite_token = Column(String(64), unique=True, nullable=False, default=lambda: secrets.token_urlsafe(32))
    ...
    # Relationships
    landlord = relationship("User", foreign_keys=[landlord_id], backref="team_members_owned")
    member = relationship("User", foreign_keys=[member_user_id], backref="team_memberships")
    property_access = relationship("TeamMemberProperty", back_populates="team_member", cascade="all, delete-orphan")
...
class TeamMemberProperty(Base):
    ...
    team_member_id = Column(UUID(as_uuid=True), ForeignKey("team_members.id"), nullable=False, index=True)
    ...
    permission_override = Column(
        SQLEnum(PermissionLevel, name='permission_level_enum', native_enum=True, create_type=False),
        nullable=True
    )
