...
from sqlalchemy import (DECIMAL, TIMESTAMP, Boolean, Column, Date, ForeignKey,
                        Integer, String, Text)
...
class Property(Base):
    ...
    charges_included = Column(
        Boolean, default=False
    )  # True=CC (charges comprises), False=HC
    charges_description = Column(
        Text
    )  # "Water, maintenance, garbage, building insurance"
    ...
    media_sessions = relationship(
        "PropertyMediaSession", back_populates="property", cascade="all, delete-orphan"
    )
    media = relationship(
        "PropertyMedia", back_populates="property", cascade="all, delete-orphan"
    )
    visit_slots = relationship(
        "VisitSlot", back_populates="property", cascade="all, delete-orphan"
    )
    leases = relationship(
