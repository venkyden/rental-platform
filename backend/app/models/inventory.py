...
class Inventory(Base):
    ...
    status = Column(SQLEnum(InventoryStatus, name='inventory_status_enum'), default=InventoryStatus.DRAFT, nullable=False)
...
class InventoryItem(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    inventory_id = Column(UUID(as_uuid=True), ForeignKey("inventories.id"), nullable=False, index=True)
