...
async def generate_lease(
    ...
    if application.status != "approved":
        raise HTTPException(
            status_code=400, detail="Can only generate lease for approved applications"
        )
    ...
    # Get tenant
    result = await db.execute(select(User).where(User.id == application.tenant_id))
...
async def download_lease_pdf(
    ...
    # Get property
    result = await db.execute(select(Property).where(Property.id == lease.property_id))
    property_obj = result.scalar_one_or_none()

    # Verify access
    if (
        property_obj.landlord_id != current_user.id
        and lease.tenant_id != current_user.id
    ):
        raise HTTPException(status_code=403, detail="Not authorized")

    # Get landlord
    landlord_result = await db.execute(
        select(User).where(User.id == property_obj.landlord_id)
    )
