...
async def get_inbox(
    status: Optional[str] = Query(
        None, description="Filter by status: active, archived, resolved"
    ),
    ...
    query = (
        query.order_by(desc(Conversation.last_message_at)).offset(offset).limit(limit)
    )
...
async def get_unread_count(
    ...
    query = select(Conversation).where(
        or_(
            ...
            and_(
                Conversation.tenant_id == current_user.id,
                Conversation.unread_count_tenant > 0,
            ),
        )
    )
...
async def get_conversation(
    ...
    # Get property
    prop = (
        await db.execute(select(Property).where(Property.id == conv.property_id))
    ).scalar_one_or_none()

    # Get users
    landlord = (
        await db.execute(select(User).where(User.id == conv.landlord_id))
    ).scalar_one_or_none()
    tenant = (
        await db.execute(select(User).where(User.id == conv.tenant_id))
    ).scalar_one_or_none()
    ...
    for msg in messages:
        sender = (
            await db.execute(select(User).where(User.id == msg.sender_id))
        ).scalar_one_or_none()
        message_responses.append(
            MessageResponse(
                ...
                sender_name=(sender.full_name or "Unknown") if sender else "Unknown",
                content=msg.content,
    ...
    return ConversationDetail(
        ...
        landlord_name=(landlord.full_name or "Unknown") if landlord else "Unknown",
        tenant_name=(tenant.full_name or "Unknown") if tenant else "Unknown",
        subject=conv.subject,
        status=conv.status,
...
async def create_conversation(
    ...
    prop = (
        await db.execute(select(Property).where(Property.id == data.property_id))
    ).scalar_one_or_none()
    if not prop:
    ...
    if prop.landlord_id != current_user.id:
        raise HTTPException(
            status_code=403, detail="Only property owner can initiate conversations"
        )

    # Find tenant by email
    tenant = (
        await db.execute(select(User).where(User.email == data.tenant_email))
    ).scalar_one_or_none()
    if not tenant:
    ...
    if existing.scalar_one_or_none():
        raise HTTPException(
            status_code=400,
            detail="Conversation already exists for this property and tenant",
        )
    ...
    return ConversationSummary(
        ...
        other_party_name=tenant.full_name or "Unknown",
