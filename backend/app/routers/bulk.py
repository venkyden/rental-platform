...
async def get_import_template(
    ...
    if format == "csv":
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": "attachment; filename=properties_template.xml"
            },
        )
        ...
        for prop in properties:
            writer.writerow(
                [
                    ...
                    (
                        str(prop.has_parking).lower()
                        if prop.has_parking is not None
                        else ""
                    ),
                    (
                        str(prop.has_balcony).lower()
                        if prop.has_balcony is not None
                        else ""
                    ),
...
async def import_properties(
    ...
    # Detect format
    is_xml = (
        file.filename.lower().endswith(".xml") or file.content_type == "application/xml"
    )
        ...
        try:
            ...
            if existing:
                ...
                existing.property_type = (
                    row.get("property_type") or existing.property_type
                )
                existing.address_line1 = (
                    row.get("address_line1") or existing.address_line1
                )
                existing.address_line2 = (
                    row.get("address_line2") or existing.address_line2
                )
