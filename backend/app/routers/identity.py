...
async def start_identity_verification(
    ...
    if not await feature_flag_service.get_flag_state(db, "identity_verification", default=True):
        raise HTTPException(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Failed to start verification: {str(e)}")
    ...
    if event["type"] == "identity.verification_session.verified":
        ...
        if user_id:
            ...
            if user:
                ...
                user.trust_score = min(100, user.trust_score + 40) # +40 for ID Verification
