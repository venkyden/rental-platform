...
async def register(request: Request, user_data: UserRegister, db: AsyncSession = Depends(get_db)):
    """Register a new user"""
    audit_logger.info(f"REGISTER_ATTEMPT email={user_data.email} ip={request.client.host}")
...
async def login(
    ...
    audit_logger.info(f"LOGIN_ATTEMPT email={form_data.username} ip={request.client.host}")
    # Find user
    result = await db.execute(select(User).where(User.email == form_data.username))
    user = result.scalar_one_or_none()
    
    if not user or not verify_password(form_data.password, user.hashed_password):
        audit_logger.warning(f"LOGIN_FAILED email={form_data.username} ip={request.client.host}")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_my_segment_config(current_user: User = Depends(get_current_user)):
    ...
    from app.core.segment_routing import get_segment_config, get_redirect_path, get_all_features, COMMON_FEATURES
