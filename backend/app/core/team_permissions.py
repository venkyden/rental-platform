...
async def get_effective_permission(
    ...
    property_access = (await db.execute(
...
async def get_accessible_properties(
    ...
    owned = await db.execute(
        select(Property.id).where(Property.landlord_id == user_id)
    )
...
def _permission_meets_minimum(
    ...
    levels = [PermissionLevel.VIEW_ONLY, PermissionLevel.MANAGE_VISITS, PermissionLevel.FULL_ACCESS]
