    ...
    if user.role == UserRole.LANDLORD:
        ...
        if action in landlord_actions and resource_owner_id and db:
            access = await check_property_manager_access(
                property_manager_id=str(user.id), landlord_id=resource_owner_id, db=db
            )
            if not access:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
...
async def check_property_manager_access(
    property_manager_id: str, landlord_id: str, db: AsyncSession
) -> bool:
    """
...
async def can_manage_property(
    user: User, property_owner_id: str, db: AsyncSession
) -> bool:
    """
    ...
    if user.role == UserRole.PROPERTY_MANAGER:
        return await check_property_manager_access(
            property_manager_id=str(user.id), landlord_id=property_owner_id, db=db
        )
