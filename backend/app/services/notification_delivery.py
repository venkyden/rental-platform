...
class NotificationDelivery:
    ...
    def __init__(self):
        ...
        self.twilio_whatsapp = os.getenv(
    ...
    async def send_sms(self, to: str, message: str) -> bool:
        ...
        if not self.twilio_sid or not self.twilio_token:
            logger.warning(
                f"Twilio not configured. Would SMS to {to}: {message[:50]}..."
            )
            return False
        ...
        try:
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    data={"From": self.twilio_phone, "To": to, "Body": message},
                )
    ...
    async def send_whatsapp(self, to: str, message: str) -> bool:
        ...
        if not self.twilio_sid or not self.twilio_token:
            logger.warning(
                f"Twilio not configured. Would WhatsApp to {to}: {message[:50]}..."
            )
            return False
...
# Email templates
def get_application_email_template(
    tenant_name: str, property_title: str, action_url: str
) -> tuple[str, str]:
...
def get_status_change_email_template(
    property_title: str, status: str, action_url: str
) -> tuple[str, str]:
    ...
    status_text = (
        "approved"
        if status == "approved"
        else "declined" if status == "rejected" else status
    )
