...
class MatchingService:
    ...
    def calculate_match_details(self, tenant: User, property: Property) -> dict:
        ...
        university_city = university_info.get('city', '').lower() if isinstance(university_info, dict) else ''
        university_name = university_info.get('university_name', '').lower() if isinstance(university_info, dict) else ''
        ...
        if location_preference and isinstance(location_preference, dict) and property.latitude and property.longitude:
        ...
        wanted_amenities = (tenant.preferences or {}).get('amenities', []) or (tenant.preferences or {}).get('must_have_amenities', [])
        wanted_landmarks = (tenant.preferences or {}).get('landmarks', []) # e.g. ['hospital', 'metro']
        
        prop_amenities = (property.amenities or []) + (property.utilities_included or [])
        ...
        prop_landmarks = str(property.nearby_landmarks or "").lower() + str(property.public_transport or "").lower()
