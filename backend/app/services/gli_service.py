...
class GLIService:
    ...
    def calculate_quote(
        ...
        if tenant_income < (monthly_rent * 3):
            is_eligible = False
            reasons.append(f"Solvency ratio too low. Tenant income ({tenant_income}€) must be at least 3x rent ({monthly_rent}€).")
        ...
        valid_until = (datetime.now() + timedelta(days=self.QUOTE_VALIDITY_DAYS)).strftime("%d/%m/%Y")
