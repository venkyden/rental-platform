    ...
    async def handle_webhook(
        ...
        if event_type == 'identity.verification_session.verified':
            ...
            await self._update_record(
                db, session_id, VerificationStatus.VERIFIED,
                {'verified_at': datetime.utcnow().isoformat()}
            )
            ...
            await self._update_record(
                db, session_id, VerificationStatus.PENDING,
                {'requires_input': True}
            )
            ...
            # Canceled
            await self._update_record(
                db, session_id, VerificationStatus.FAILED,
                {'canceled': True}
            )
    ...
    async def _update_record(
        ...
        result = await db.execute(
            select(VerificationRecord).where(
                VerificationRecord.verification_data['stripe_session_id'].astext == session_id
            )
        )
