...
class GLIService:
    ...
    def check_eligibility(
        self, monthly_rent: float, tenant: TenantProfile
    ) -> tuple[bool, str]:
        """
        ...
        if income_ratio < self.MIN_INCOME_RATIO:
            return (
                False,
                f"Revenu insuffisant. Ratio actuel: {income_ratio:.1f}x (minimum: {self.MIN_INCOME_RATIO}x)",
            )
        ...
        if monthly_rent > self.MAX_RENT_COVERED:
            return (
                False,
                f"Loyer trop élevé. Maximum couvert: {self.MAX_RENT_COVERED}€/mois",
            )
    ...
    def calculate_premium(self, monthly_rent: float, tenant: TenantProfile) -> Dict:
        """
