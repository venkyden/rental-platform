...
class LeaseGenerator:
    ...
    def _draw_parties(self, c: canvas.Canvas, y: float, landlord: User, tenant: User) -> float:
    ...
    def _draw_property_section(self, c: canvas.Canvas, y: float, property: Property) -> float:
        ...
        y -= 0.5*cm
        c.drawString(2.5*cm, y, f"Surface habitable (Loi Carrez): {property.size_sqm} m²")
        y -= 0.5*cm
    ...
    def _draw_duration_section(self, c: canvas.Canvas, y: float, config: dict, start: datetime, end: datetime, duration: int) -> float:
        ...
        c.drawString(2.5*cm, y, f"Date de prise d'effet: {start.strftime('%d/%m/%Y')}")
        y -= 0.5*cm
        ...
        if config['renewable']:
            c.drawString(2.5*cm, y, "Renouvellement: Tacite reconduction à l'échéance du bail")
        else:
            c.drawString(2.5*cm, y, "Renouvellement: Non renouvelable (bail à durée déterminée)")
        
        return y - 1*cm

    def _draw_financial_section(self, c: canvas.Canvas, y: float, rent: float, deposit: float, charges: float, config: dict) -> float:
        ...
        if deposit > 0:
            ...
            c.drawString(2.5*cm, y, f"Dépôt de garantie: {deposit:.2f} € ({deposit_months} mois max légal)")
        else:
            c.drawString(2.5*cm, y, "Dépôt de garantie: Non applicable (bail mobilité)")
        y -= 0.5*cm
        
        c.drawString(2.5*cm, y, "Modalités de paiement: À terme échu, le 1er de chaque mois")
        
        return y - 1*cm

    def _draw_notice_section(self, c: canvas.Canvas, y: float, config: dict, lease_type: str) -> float:
        ...
        if lease_type == 'mobilite':
            c.drawString(2.5*cm, y, f"• Délai: {tenant_notice} mois avant la date de départ souhaitée")
            y -= 0.4*cm
            c.drawString(2.5*cm, y, "• Le locataire peut partir à tout moment avec préavis")
        else:
            ...
            if lease_type in ['meuble', 'vide']:
                ...
                y -= 0.35*cm
                c.drawString(3*cm, y, "- État de santé justifiant un changement de domicile")
                y -= 0.35*cm
        ...
        if landlord_notice == 0:
            c.drawString(2.5*cm, y, "• Le bailleur ne peut pas donner congé avant l'échéance du bail")
        else:
            c.drawString(2.5*cm, y, f"• Délai: {landlord_notice} mois avant l'échéance du bail")
            y -= 0.4*cm
            c.drawString(2.5*cm, y, "• Motifs légaux uniquement:")
            y -= 0.35*cm
            c.setFont("Helvetica", 9)
            c.drawString(3*cm, y, "- Reprise pour habiter (bailleur ou famille proche)")
            y -= 0.35*cm
            c.drawString(3*cm, y, "- Vente du logement")
            y -= 0.35*cm
            c.drawString(3*cm, y, "- Motif légitime et sérieux (manquements du locataire)")
            c.setFont("Helvetica", 10)
        
        y -= 0.6*cm
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(2.5*cm, y, "Le congé doit être notifié par lettre recommandée avec AR ou par acte d'huissier.")
        
        return y - 1*cm

    def _draw_end_of_lease_section(self, c: canvas.Canvas, y: float, config: dict, lease_type: str, end_date: datetime) -> float:
        ...
        if config['renewable']:
            c.drawString(2.5*cm, y, "À l'échéance, le bail est reconduit tacitement pour la même durée,")
            y -= 0.4*cm
            c.drawString(2.5*cm, y, "sauf congé donné dans les délais légaux.")
        else:
            c.drawString(2.5*cm, y, "Le bail prend fin de plein droit à l'échéance sans qu'il soit")
            y -= 0.4*cm
        ...
        y -= 0.4*cm
        c.drawString(2.5*cm, y, "• Dépôt de garantie restitué sous 1 mois (sans dégradations)")
        y -= 0.4*cm
    ...
    def _draw_obligations_section(self, c: canvas.Canvas, y: float) -> float:
        ...
        y -= 0.35*cm
        c.drawString(2.5*cm, y, "• Entretenir les locaux et faire les réparations nécessaires")
        y -= 0.35*cm
        c.drawString(2.5*cm, y, "• Fournir les diagnostics obligatoires (DPE, CREP, Amiante, etc.)")
        ...
        y -= 0.35*cm
        c.drawString(2.5*cm, y, "• Répondre des dégradations survenues pendant la location")
        y -= 0.35*cm
    ...
    def _draw_etat_des_lieux_section(self, c: canvas.Canvas, y: float) -> float:
        ...
        y -= 0.5*cm
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(2.5*cm, y, "L'état des lieux doit être annexé au présent contrat.")
    ...
    def _draw_insurance_section(self, c: canvas.Canvas, y: float) -> float:
        ...
        c.drawString(2.5*cm, y, "Le locataire est tenu de s'assurer contre les risques locatifs")
        y -= 0.4*cm
        c.drawString(2.5*cm, y, "(incendie, dégât des eaux, explosion) et d'en justifier annuellement")
        y -= 0.4*cm
        c.drawString(2.5*cm, y, "auprès du bailleur.")
        y -= 0.5*cm
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(2.5*cm, y, "À défaut, le bailleur peut souscrire une assurance pour le compte")
        y -= 0.35*cm
    ...
    def _draw_termination_clause(self, c: canvas.Canvas, y: float) -> float:
        ...
        y -= 0.4*cm
        c.drawString(2.5*cm, y, "• En cas de troubles de voisinage constatés par décision de justice")
        y -= 0.5*cm
        c.setFont("Helvetica-Oblique", 9)
        c.drawString(2.5*cm, y, "Après un commandement de payer resté infructueux pendant 2 mois.")
    ...
    def _draw_signatures(self, c: canvas.Canvas, y: float, start: datetime):
        ...
        c.setFont("Helvetica", 10)
        c.drawString(2*cm, y, f"Fait en deux exemplaires originaux, le {start.strftime('%d/%m/%Y')}")
        y -= 0.5*cm
    ...
    def _draw_footer(self, c: canvas.Canvas, config: dict):
        ...
        c.drawString(2*cm, 1.5*cm, f"Document établi conformément à la {config['law_reference']}.")
        c.drawString(2*cm, 1.1*cm, "Ce contrat type est conforme aux dispositions de l'annexe au décret n° 2015-587 du 29 mai 2015.")
        c.drawString(2*cm, 0.7*cm, "Document généré automatiquement. Les parties sont invitées à vérifier la conformité avec leur situation particulière.")
