...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "landlord_id", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=False
        ),
        sa.Column(
            "tenant_id", UUID(as_uuid=True), sa.ForeignKey("users.id"), nullable=True
        ),
    ...
    op.create_index(
        op.f("ix_visit_slots_property_id"), "visit_slots", ["property_id"], unique=False
    )
    op.create_index(
        op.f("ix_visit_slots_start_time"), "visit_slots", ["start_time"], unique=False
    )

    # --- Lease Types Enum ---
    lease_type_enum = sa.Enum(
        "meuble", "vide", "mobilite", "etudiant", name="lease_type_enum"
    )
    # try:
    ...
    op.create_table(
        ...
        ),  # Nullable for CDI (3 years auto-renew)
        ...
        sa.Column(
            "lease_type",
            sa.Enum("meuble", "vide", "mobilite", "etudiant", name="lease_type_enum"),
            nullable=False,
        ),
        sa.Column(
            "status", sa.String(), default="draft"
        ),  # draft, signed, active, terminated
        sa.Column("pdf_path", sa.String(), nullable=True),  # Path to generated PDF
        sa.Column(
            "signature_data", JSONB, nullable=True
        ),  # Electronic signature metadata
        sa.Column("created_at", sa.DateTime(), server_default=sa.func.now()),
        sa.Column("updated_at", sa.DateTime(), server_default=sa.func.now()),
    )
    op.create_index(
        op.f("ix_leases_property_id"), "leases", ["property_id"], unique=False
    )
