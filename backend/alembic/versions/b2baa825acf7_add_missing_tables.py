...
def upgrade() -> None:
    ...
    op.create_unique_constraint('onboarding_responses_user_id_key', 'onboarding_responses', ['user_id'])
    ...
    op.add_column('properties', sa.Column('surface_type', sa.String(length=20), nullable=True))
    ...
    op.create_foreign_key('property_media_sessions_property_id_fkey', 'property_media_sessions', 'properties', ['property_id'], ['id'])
    ...
    op.alter_column('users', 'employment_data',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               type_=sa.JSON(),
               existing_nullable=True)
...
def downgrade() -> None:
    ...
    op.add_column('visit_slots', sa.Column('updated_at', postgresql.TIMESTAMP(), server_default=sa.text('now()'), autoincrement=False, nullable=True))
    op.add_column('visit_slots', sa.Column('created_at', postgresql.TIMESTAMP(), server_default=sa.text('now()'), autoincrement=False, nullable=True))
    op.alter_column('verification_records', 'verification_data',
    ...
    op.alter_column('users', 'identity_data',
               ...
               type_=postgresql.JSONB(astext_type=sa.Text()),
               existing_nullable=True)
    op.create_index('ix_team_members_invite_token', 'team_members', ['invite_token'], unique=False)
    op.create_index('ix_team_members_email', 'team_members', ['email'], unique=False)
    op.drop_constraint('property_media_sessions_property_id_fkey', 'property_media_sessions', type_='foreignkey')
    op.create_foreign_key('property_media_sessions_property_id_fkey', 'property_media_sessions', 'properties', ['property_id'], ['id'], ondelete='CASCADE')
    ...
    op.drop_constraint('onboarding_responses_user_id_key', 'onboarding_responses', type_='unique')
    op.create_index('idx_onboarding_user_id', 'onboarding_responses', ['user_id'], unique=False)
    op.alter_column('onboarding_responses', 'responses',
               existing_type=postgresql.JSONB(astext_type=sa.Text()),
               nullable=False)
    op.alter_column('onboarding_responses', 'user_id',
    ...
    op.add_column('leases', sa.Column('updated_at', postgresql.TIMESTAMP(), server_default=sa.text('now()'), autoincrement=False, nullable=True))
    op.add_column('leases', sa.Column('created_at', postgresql.TIMESTAMP(), server_default=sa.text('now()'), autoincrement=False, nullable=True))
