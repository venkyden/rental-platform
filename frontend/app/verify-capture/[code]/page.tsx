...
export default function VerifyCapturePage() {
    ...
    const documentTypes = [
        { value: 'passport', label: t('verify.identity.passport', undefined, 'Passport'), description: 'Bio page with photo', icon: '🌍' },
        { value: 'id_card', label: t('verify.identity.idCard', undefined, 'ID Card'), description: 'Front & Back photo', icon: '🆔' },
        { value: 'drivers_license', label: t('verify.identity.driversLicense', undefined, 'Driver\'s License'), description: 'Front & Back photo', icon: '🚗' },
        { value: 'residence_permit', label: t('verify.identity.residencePermit', undefined, 'Residence Permit'), description: 'Front & Back photo', icon: '🏠' },
    ];
    ...
    const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            setStep('loading'); // Show mini loader
            try {
                let processedFile: File | Blob = rawFile;
                if (!isHeic) {
                    processedFile = await compressImage(rawFile);
                } else {
                    toast.success('HEIC image detected. Bypassing client-side operations.');
                }
            ...
            <header className="px-6 py-6 flex items-center justify-between relative z-10">
                <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.3em] text-zinc-900">
                        {language === 'fr' ? 'Capture Sécurisée' : 'Secure Capture'}
                    </span>
                </div>
                    ...
                    {step === 'error' && (
                        ...
                        >
                            ...
                            <h1 className="text-3xl font-black uppercase tracking-tighter mb-4 text-zinc-900">
                                {language === 'fr' ? 'Lien Expiré' : 'Session Expired'}
                            </h1>
                            <p className="text-zinc-500 font-bold mb-8 px-4">
                                {language === 'fr' 
                                    ? 'Lien de vérification invalide ou expiré. Veuillez générer une nouvelle session.'
                                    : errorMessage
                                }
                            </p>
                            <button 
                            ...
                            >
                                {language === 'fr' ? 'Actualiser' : 'Try Refreshing'}
                            </button>
                        </motion.div>
                    )}
                    ...
                    {step === 'success' && (
                        ...
                        >
                            ...
                            <p className="text-zinc-500 font-bold text-lg leading-relaxed mb-12">
                                {language === 'fr'
                                    ? 'Votre identité a été transmise en toute sécurité. Vous pouvez maintenant retourner sur votre ordinateur.'
                            ...
                            <div className="w-full p-6 rounded-3xl bg-zinc-900 text-white shadow-xl">
                                <p className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">
                                    {language === 'fr' ? 'Synchronisation active' : 'Desktop sync active'}
                                </p>
                            </div>
                        </motion.div>
                    )}
                        ...
                        >
                            <div>
                                <h2 className="text-4xl font-black uppercase tracking-tighter mb-2 text-zinc-900 leading-none">
                                    {side === 'bio' ? (language === 'fr' ? 'Identité' : 'Identity') : side === 'front' ? (language === 'fr' ? 'Face Avant' : 'Front Side') : (language === 'fr' ? 'Face Arrière' : 'Back Side')}
                                </h2>
                                <p className="text-zinc-500 font-medium">
                                    {side === 'bio' 
                                        ? (language === 'fr' ? 'Sélectionnez un document pour commencer le scan' : 'Select document to begin scan') 
                                        : (language === 'fr' ? `Prenez en photo le côté : ${side}` : `Capture the ${side} of your document`)
                                    }
                                </p>
                            </div>
                                ...
                                {side === 'bio' ? (
                                    ...
                                    <div className="p-8 rounded-[2rem] border-2 border-dashed border-zinc-900/30 bg-zinc-900/5 flex flex-col items-center justify-center text-center">
                                        ...
                                        <p className="text-sm font-black text-zinc-900 mb-2 uppercase tracking-tighter">
                                            {language === 'fr' ? `Prêt pour la face : ${side}` : `Ready for ${side} side`}
                                        </p>
                                        <p className="text-xs text-zinc-500 font-bold">
                                            {language === 'fr' ? 'Évitez les reflets et ombres' : 'Ensure lighting is clear and no glare'}
                                        </p>
                                    </div>
                                )}
                            </div>
                            ...
                            >
                                ...
                                {language === 'fr' ? 'Lancer l’Appareil' : 'Launch Camera'}
                            </button>
                        </motion.div>
                    )}
                    ...
                    {step === 'preview' && previewUrl && (
                        ...
                        >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                >
                                    {language === 'fr' ? 'Valider' : 'Confirm'} <ArrowRight className="w-4 h-4" />
                                </button>
                            </div>
                        </motion.div>
                    )}
                    ...
                    {step === 'uploading' && (
                        ...
                        >
                            ...
                            <h3 className="text-3xl font-black uppercase tracking-tighter mb-2 text-zinc-900">
                                {language === 'fr' ? 'Envoi...' : 'Transmitting'}
                            </h3>
                            <p className="text-zinc-500 font-medium">
                                {language === 'fr' ? 'Sécurisation de vos données...' : 'Securing your identity data...'}
                            </p>
                        </motion.div>
                    )}
                </AnimatePresence>
