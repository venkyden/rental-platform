...
export default function DashboardPage() {
    ...
    const quickActions = [
        {
            icon: <Home className="w-8 h-8 text-zinc-900" />,
        ...
        {
            icon: <ShieldCheck className="w-8 h-8 text-zinc-900" />,
        ...
        {
            icon: <Mail className="w-8 h-8 text-zinc-900" />,
        ...
        {
            icon: <UserCircle className="w-8 h-8 text-zinc-900" />,
            ...
            <PremiumLayout withNavbar={true}>
                ...
                >
                    ...
                    <motion.div variants={itemVariants} className="relative group">
                        <div className="glass-card !p-0 overflow-hidden border-none shadow-[0_32px_64px_-16px_rgba(0,0,0,0.1)] rounded-[3rem]">
                            <div className="absolute inset-0 bg-gradient-to-br from-zinc-200/20 via-transparent to-zinc-200/20 group-hover:opacity-100 opacity-40 transition-opacity duration-1000" />
                    ...
                    {(user.role === 'landlord' || user.role === 'property_manager') && (
                        <motion.div variants={containerVariants} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                            {loadingStats ? (
                                ...
                                <>
                                    {[
                                        { label: t('dashboard.stats.properties', undefined, undefined), value: stats.total, color: 'text-zinc-900', sub: t('dashboard.stats.propertiesDesc', undefined, undefined), icon: <Building className="w-4 h-4" /> },
                                        { label: t('dashboard.stats.activeListings', undefined, undefined), value: stats.active, color: 'text-zinc-900', sub: t('dashboard.stats.activeDesc', undefined, undefined), icon: <CheckCircle2 className="w-4 h-4" /> },
                                        { label: t('dashboard.stats.drafts', undefined, undefined), value: stats.draft, color: 'text-zinc-400', sub: t('dashboard.stats.draftsDesc', undefined, undefined), icon: <Clock className="w-4 h-4" /> },
                                        { label: t('dashboard.stats.views', undefined, undefined), value: stats.total_views, color: 'text-zinc-500', sub: t('dashboard.stats.viewsDesc', undefined, undefined), icon: <BarChart3 className="w-4 h-4" /> }
                                    ].map((stat, i) => (
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                            {quickActions.filter(action => action.show).map((action, idx) => (
                                <motion.button
                                    ...
                                    className="glass-card !p-10 flex flex-col items-start text-left group border-zinc-100 hover:border-zinc-900 transition-all duration-700 relative overflow-hidden rounded-[2.5rem] shadow-xl"
                                >
                                    <div className="absolute top-0 right-0 w-32 h-32 bg-gradient-to-bl from-zinc-900/5 to-transparent rounded-bl-[100%] opacity-0 group-hover:opacity-100 transition-opacity duration-700" />
                                    
                                    <div className="mb-8 p-5 rounded-2xl bg-zinc-50 group-hover:bg-zinc-900 text-zinc-900 group-hover:text-white transition-all duration-700 shadow-lg group-hover:shadow-zinc-900/20 group-hover:-rotate-6">
                                        {action.icon}
                                    </div>
                                    <h4 className="text-3xl font-black text-zinc-900 mb-3 tracking-tighter uppercase group-hover:text-zinc-900 transition-colors">{action.title}</h4>
                                    <p className="text-zinc-500 font-medium leading-relaxed text-lg">{action.description}</p>
                                    {action.badge && (
                                        <div className="absolute top-10 right-10 w-12 h-12 bg-zinc-900 flex items-center justify-center text-white text-xs font-black rounded-2xl shadow-2xl group-hover:scale-110 transition-transform">
                                            {action.badge}
                                        </div>
                                    )}
                                </motion.button>
                                    ...
                                    <div className="divide-y divide-zinc-100">
                                        {recentConversations.map((conv) => (
                                            <button
                                                key={conv.id}
                                                onClick={() => router.push('/inbox')}
                                                className="w-full p-10 text-left hover:bg-zinc-50/50 transition-all flex items-center gap-8 group"
                                            >
                                                <div className="w-16 h-16 rounded-[1.5rem] bg-zinc-900 text-white flex items-center justify-center text-2xl font-black shadow-xl group-hover:scale-110 group-hover:rotate-3 transition-all duration-500 relative overflow-hidden">
                                                    <div className="absolute inset-0 bg-gradient-to-br from-white/10 to-transparent" />
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-zinc-500 text-base truncate font-medium">
                                    ...
                                    {[
                                        ...
                                        <div key={i} className="flex items-center justify-between group">
                                            <div className="flex items-center gap-5">
                                                <div className={`w-10 h-10 rounded-2xl flex items-center justify-center transition-all duration-500 ${item.verified ? 'bg-zinc-900 text-white shadow-lg shadow-zinc-900/30' : 'bg-zinc-100 text-zinc-400'}`}>
                                    ...
                                    <div className="relative w-48 h-48">
                                        <svg className="transform -rotate-90 w-full h-full">
                                            ...
                                            <motion.circle 
                                                ...
                                                cx="96" cy="96" r="88" stroke="url(#zinc-gradient)" strokeWidth="16" fill="none" strokeLinecap="round" 
                                            />
                                            <defs>
                                                <linearGradient id="zinc-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
