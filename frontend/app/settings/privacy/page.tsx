                        ...
                        <div className="flex flex-row md:flex-col gap-2 p-1.5 bg-zinc-100 rounded-full md:rounded-[2rem] border border-zinc-200/50 backdrop-blur-xl overflow-x-auto no-scrollbar scroll-smooth">
                            {[
                                ...
                                <div key={tab.id} className="flex flex-row md:flex-col shrink-0">
                                    ...
                                    >
                                        <tab.icon className={`w-3.5 h-3.5 md:w-4 md:h-4 ${tab.id === 'privacy' ? 'text-zinc-100' : ''}`} />
                                        {tab.label}
                                    </button>
                        ...
                        <div className="space-y-12">
                            <div className="glass-card !p-10">
                                <div className="flex items-center gap-4 mb-8">
                                    <div className="p-3 bg-zinc-900/10 rounded-2xl">
                                ...
                                <div className="p-8 bg-zinc-50 rounded-[2rem] border border-zinc-100">
                                    <div className="flex items-center gap-4 mb-6">
                                        <div className="p-3 bg-zinc-900/10 rounded-xl">
                                            <ShieldAlert className="w-5 h-5 text-zinc-900" />
                                        </div>
                                        <h3 className="text-sm font-black uppercase tracking-widest text-zinc-900">{t('settings.privacy.dangerTitle')}</h3>
                                    </div>
            ...
            <AnimatePresence>
                {showDeleteModal && (
                    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-zinc-950/80 backdrop-blur-xl z-[100] flex items-center justify-center p-6">
                        <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="bg-white rounded-[2.5rem] max-w-md w-full p-10 shadow-2xl relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-6">
                                <button onClick={() => setShowDeleteModal(false)} className="p-2 hover:bg-zinc-100 rounded-xl transition-all"><X className="w-5 h-5 text-zinc-400" /></button>
                            </div>
                            
                            <div className="mb-8">
                                <div className="w-16 h-16 bg-zinc-100 rounded-2xl flex items-center justify-center mb-6">
                                    <Trash2 className="w-8 h-8 text-zinc-900" />
                                </div>
                            ...
                            <div className="space-y-6">
                                <div className="space-y-2">
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-zinc-50 border-none rounded-2xl px-6 py-4 text-center font-black text-zinc-900 uppercase tracking-widest focus:ring-2 focus:ring-zinc-900/20 transition-all"
                                    />
                                </div>
