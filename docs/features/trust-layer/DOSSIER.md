...
16. **E-sign legal gate CLEARED 2026-06-24.** Lawyer green-lit **both** paths:
    ...
    contract type → e-sign. ✅ **Written opinion now on file (2026-06-20, Mathieu Galand,
    Avocat au Barreau de Paris)** — `docs/legal/2026-06-20-avis-avocat-esign-lease-galand.md`;
    closes the PRD §7.6 "obtain the blessing in writing" item. The opinion deems both modules
    permissible and "suitable for commercial deployment," **subject to** §3 conditions (mandatory
    provisions; annexes/diagnostics; identity verification; auditable signing record; GDPR) — the
    ...
    **Path B** (upload + e-sign, lower risk) before **Path A** (Décret 2015-587 template
    ...
    wording only (no custom clauses, loi 1971), any third-party AGPL e-sign tool run
    unmodified + self-hosted, no success fee, never touch the deposit/funds. eIDAS simple/
...
| SG-5 | Party wants max robustness | offer **QTSP** upgrade (paid, v2) | ⏸ deferred (QTSP) by design |
...
- **Engine decision: in-house Ed25519, not a third-party AGPL e-sign tool** (founder constraint:
  0 opex/capex — no second service to host, no AGPL surface). Reuses the credential signing key
  via new `CredentialService.sign_payload`/`verify_payload`, so the **published public key
  verifies both credentials and lease signatures**. A QTSP upgrade stays the deferred SG-5 v2 path.
...
- **§5.6 legality red-line shipped** (`lease_legality.py`, deterministic): on upload, screens the
  PDF text → **VALIDATED** vs **ATTACHED**; LU-1 (no text layer), LU-2 (art. 4 prohibited-clause
...
- **Out of v1 / next:** LU-3 deposit-cap + AI clause extraction (§5.6); Path A template generation
  (§5.5, legally gated by the opinion's mandatory-provisions + annexes conditions); QTSP upgrade.
...
9. 🟡 **E-sign + evidence pack** (§5.7, §6) — ✅ **SHIPPED 2026-06-24 as in-house Ed25519** (not a third-party AGPL e-sign tool — founder 0 opex/capex; reuses the credential signing key so one published public key verifies both). eIDAS simple/advanced sig valid for a bail; gate CLEARED 2026-06-24 (§0.16). A QTSP upgrade is **deferred** to the SG-5 "max robustness" v2 path. SG-1..SG-4 enforced + tested (§5.7).
...
13. ✅ **INTL fiscal-capacity (funds) solvency + plain-language assurance** — **shipped 2026-06-17** (branch `feat/intl-fiscal-capacity`). `POST /verification/intl/funds`: bank statement / scholarship / sponsorship / loan-approval → FX-normalised banded `funds_coverage` (MEDIUM, funds-not-income, self-contained, no third party). `band_funds_coverage` + `_name_present` + `_ai_extract_intl_funds` helpers; anti-fraud flags (name_present, duration_covers_lease); statelessness verified (no raw amount persisted). Sponsor funds = fiscal-capacity signal (`funds_source: sponsor`), not guarantor brokering. `issue_mine` emits `funds_coverage_*` via extracted `_build_claims_for_user`. Plain-language assurance rendering shipped on the verify page (`/c/`, dropped HIGH/MEDIUM/amber badges) + evidence PDF (`_verified_phrase`, no tier words) + funds row. Spec: `docs/superpowers/specs/2026-06-17-intl-fiscal-capacity-solvency-design.md`; Plan: `docs/superpowers/plans/2026-06-17-intl-fiscal-capacity-solvency.md`. **Still pending: item 14 (INTL solvency/funds UI tab) — endpoint has no frontend entry point yet.**
...
15. ✅ **Deposit-binding evidence layer** — **shipped 2026-07-12** (branch `feat/deposit-binding-sci`). `POST /verification/deposit/bind`: verified landlord ↔ property ↔ deposit amount ↔ **payee IBAN (masked) + name-match verdict** ↔ date bound into the signed evidence doc (`deposit_binding_data` column, own column not `ownership_data`). **VoP decision:** bank-side VoP is not free/callable and edges the funds flow, so the check is Roomivo-side — offline IBAN mod-97 validation (`services/iban.py`) + `fr_2ddoc.name_matches_any` of the declared holder vs the verified landlord (or SCI, item 16); `bank_ownership_confirmed: false` is disclosed everywhere (endpoint note, evidence PDF, verify page). Deposit cap reused from `lease_rules.validate_deposit` (LG-1/LG-2, mobilité=0). GDPR: masked IBAN + verdict only at rest — raw IBAN/holder name never persisted (emit-and-forget); erasure + Art. 20 export both cover the column. Verify page renders MATCH green / MISMATCH amber ("n'envoyez rien"). 25 item-15 tests. Roomivo not in the money flow (§0.15). *Deferred:* tenant self-attested bank-VoP result field.
16. ✅ **Entity / SCI landlord verification** — **shipped 2026-07-12** (same branch). `POST /verification/landlord-entity/verify` with a **landlord-type branch** (individual / sci / manager). **B0 confirmed** the free `recherche-entreprises.api.gouv.fr` returns `dirigeants[]` (`{nom, prenoms, qualite:"Gérant"}`) + `nature_juridique` (SCI=6540) → **no INPI/RNE call needed**. `french_gov_service.verify_entity(siren)` + gérant-name match chains verified gérant → SCI `denomination`; sets `kbis_verified`; the item-15 payee match then targets the SCI denomination (entity↔entity). `manager` (Hoguet carte G) sets `carte_g_verified` scaffolding only — **no mandate/brokering flow** (entremise red line). Only public denomination + match booleans at rest. Verify page shows "Bailleur : SCI [denomination] — gérant vérifié ✓". 10 item-16 tests.

### Suggested build order (back-to-school critical path)
1. **Launch prerequisites:** funded Gemini key (§0.14); privacy policy + consent + Mentions Légales (Google subprocessor disclosure **shipped** on `feat/beta-launch-prereqs`; no-SIRET status, §0.18); wire the already-authorized PÉPITE/SNEE/Ministère logos (§0.10) into verify/landing/anti-phishing surfaces (**blocked on logo image assets**). GDPR-page "entirely in the EU" claim **softened** (same branch) to match the Gemini data flow.
   - **Roadmap — Gemini → Vertex AI (EU region):** the current AI Studio Gemini API does not guarantee EU data residency. Migrate the `_ai_extract_*` layer to **Vertex AI with an EU regional endpoint** to get genuine EU residency + an enterprise DPA, which restores the strong "EU-built" claim and clears B2B-insurer diligence. Not launch-blocking; do before scaling / insurer pitch. (Distinct from the AI-Act question, §0.19.)
2. **Item 13 + 14** — INTL funds rail + its UI (makes the loop actually serve students).
3. ✅ **Item 15 + 16** — deposit-binding + entity/SCI (the deposit-safety story) — **shipped 2026-07-12**, branch `feat/deposit-binding-sci`.
...
| INTL identity HIGH | Native passport NFC read — vendor TBD, deferred to EUDI Wallet (§0.12) | Needs CSCA list; check license before picking one |
...
| E-signature | In-house Ed25519 (shipped 2026-06-24, §5.7) | Reuses the credential signing key; QTSP upgrade deferred to v2 |
