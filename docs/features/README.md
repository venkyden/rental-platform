...
| Trust Layer (verify→sign→proof) | `verification.py`, `credentials.py`, `credential.py`, `fr_2ddoc.py`, `identity_assurance.py`, `dpe_compliance.py` | `/credentials/*`, `/verification/*`, `/c/[id]` | [trust-layer](trust-layer/DOSSIER.md) | ✅ Phase 1 complete (GLI→credential→identity→solvency→property→wiring); DPE/guarantor shipped; open: lease/e-sign (legal gate), INTL, statelessness |
| Guarantor / Visale | `verification.py` (visale), `guarantor_compliance.py` | `/verify/guarantor` | [verification-kyc](verification-kyc/journal.md) | ✅ fixed (2026-06-12): cert extractor, expiry check, assurance tier, physical submit endpoint |
